...
func main() {
	// Так как разрешены только встроенные пакеты, то с чтением из env заморачиваться не буду, взял бы "github.com/caarlos0/env".
	cfg := &config.Config{
		PortHTTP:                "8080",      // Порт на котором работает HTTP сервер
		RequestLimitCount:       100,         // Сколько максимум запросов обслуживает сервер
